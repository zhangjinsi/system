...
class Connection(object):
    ...
    def execute_commit(self,sql=None):
        ...
        '''
        try:
            conn = pymysql.connect(host=self.host, port=self.port, user=self.user, password=self.password, db=self.database, charset=self.chartset, cursorclass=DictCursor)
...
if __name__ == '__main__':
    db = Connection(host='192.168.1.69',port=3306,user='root',password='root',database='xxxSystem')
